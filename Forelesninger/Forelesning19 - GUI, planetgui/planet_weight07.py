...
class Hovedvindu(QWidget):
    def __init__(self):
        ...
        # Oppretter et skjema av typen QFormLayout, og legger til widgets for å ta input fra brukeren. skjema skal så plasseres i gridlayout (linje 55)
        ...
        for planet in planeter:                                     # Løkken fyller så inn valg for her av planetene som fins i planeter-lista
            self.meny_combobox.addItem(planet)                      #      Istedenfor en for-løkke kunne vi også brukt self.meny_combobox.addItems(planeter) for å fylle inn valgene
        self.meny_combobox.activated.connect(self.oppdater_bilde)   # Legger til en hendelse og kobler den til en funksjon/metode. Hver gang bruker velger i lista skal metoden oppdater_bilde kalles. 
                                                                    #      Merk at dog () IKKE er med her, for da ville metoden blitt forsøkt kjørt umiddelbart og vi ville fått en feilmelding.
        ...
        self.skjema.addRow(self.vekt_input)                         # Legger til i skjemaet
        ...
        self.pixmap = self.pixmap.scaled(256, 256)                  # skalerer bildet (antall piksler i parentes)
    ...
    def regn_ut(self):
        ...
        if self.planetnummer == 0:
            self.planetnummer = random.randrange(0, len(planeter))     # Genererer nytt tilfeldig indeksnummer og overskriver variabelen med dette
        ...
        # Vi kaller på en ny funksjon (som defineres under) for å oppdatere teksten. Bildet er allerede korrekt og håndtert tidligere i en hendelse tilknyttet comboboksen
        else:
            self.oppdater_tekst()
...
# Vi gjenbruker funksjonen under fra et tidligere eksempel (planet_weight_5.py). Merk at denne ikke tilhører klassen vår (intet innrykk). 
